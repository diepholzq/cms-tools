...
#     
    ...
    bins_total = np.intc([pMSSMid1_nbins, pMSSMid2_nbins])
    lowedges_total = np.float64([pMSSMid1_low, pMSSMid2_low])
    upedges_total = np.float64([pMSSMid1_up, pMSSMid2_up])
    thnsparse = THnSparseD("pMSSM_ids","pMSSM_ids",2,bins_total,lowedges_total,upedges_total) #the third argument is the dimensionality of the THnSparse object
    ...
    for ientry in range(nentries):
        ...
        pMSSMid1 = c.SusyMotherMass
        pMSSMid2 = c.SusyLSPMass

        vars["pMSSMid1"][0] = pMSSMid1
        vars["pMSSMid2"][0] = pMSSMid2
        vars["Weight"][0] = 1
        coordinates = np.float64([pMSSMid1,pMSSMid2])
        weight = 1
...
#             if btags > 0: continue
                    ...
                    for lepIso in ["CorrJetNoMultIso"]:#["CorrJetIso", "CorrJetNoMultIso"]:
                        ...
                        for ptRange in utils.leptonCorrJetIsoPtRange:
                            ...
                            for drCut in utils.leptonCorrJetIsoDrCuts:
                                ...
                                ### REVIEW CR
                                if mini is not None and mini < drCut and jetsObs["Jets"][isoJetsIdx[lep][lepIso+str(ptRange)][minCan]].Pt() < 30: #and min >= 0.2
        ...
        if not data:    
            ...
            vars["ElSfNom"][0], vars["MuSfNom"][0] = 1, 1#SB
